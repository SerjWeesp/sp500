...
def handle_rate_limit(response, timeout=20):
    if 'Retry-After' in response.headers:
        ...
        try:
            ...
            if wait_time > 0:
                print(
                    f"Rate limit hit. Waiting until {retry_time} ({wait_time:.0f} seconds)...")
                time.sleep(wait_time)
    else:
...
# financial data
variables = ['eps-earnings-per-share-diluted',
             'market-cap', 'shares-outstanding']

headers = {
    # Include Brotli for better compression support
    'Accept-Encoding': 'gzip, deflate, br',
    'Accept-Language': 'en-US,en;q=0.9',     # Adjusted language preference
    'Upgrade-Insecure-Requests': '1',
    # Updated User-Agent for a recent browser version
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Safari/537.36',
    # Updated MIME types, including modern image formats
        ...
        if response_text:
            ...
            if ticker_fin:
                ...
                financial_df = pd.concat(
                    [financial_df, ticker_fin], ignore_index=True)
                print(
                    f"{count}: {name} is done. Total number of records: {len(financial_df)}")
            else:
