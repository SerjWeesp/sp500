...
def load_cik_mapping(file_path):
    ...
    cik_df["CIK"] = cik_df["CIK"].astype(str).str.zfill(10)  # Ensure 10-digit CIK
...
def get_sec_filing_dates(tickers, cik_dict, form_type="10-Q", count=10):
    ...
    for ticker in tickers:
        ...
        df = pd.DataFrame(ten_q_filings[:count], columns=["Form Type", "Filing Date"])
        df['Ticker'] = ticker
        merged_df = pd.concat([merged_df, df], axis=0)
    return merged_df

# Load and display the CIK mapping
cik_dict = load_cik_mapping(file_path="C:/Users/Pavilion/ticker.txt")
count = 1

dates_df = pd.DataFrame()
for ticker in tickers:   
    ticker_df = get_sec_filing_dates(ticker, cik_dict = cik_dict, form_type="10-Q", count=None)
